...
describe('结算支付功能测试', () => {
  ...
  it('TC-PAY-008 【正常】Stripe沙箱支付成功返回PAID订单', async () => {
  ...
  it('TC-PAY-012 【正常】PayPal回调成功更新订单为PAID', async () => {
    const mockPaypalCallback = vi.fn().mockResolvedValue({
      code: 200, data: { orderId: 'ORD-2024-002', status: 'PAID' }
    })
  ...
  it('TC-PAY-014 【异常】重复提交支付应被幂等拦截', async () => {
    let callCount = 0
    const mockIdempotentPay = vi.fn().mockImplementation(async () => {
      callCount++
